...
%% Lemma for S_j
\begin{lemma}{
	Assuming that the error rate is independent and
	the distribution of $\Gamma(j)$ is i.i.d for all stabilizers $j$,
	$S_1, S_2, ..., S_M$ are independent and
	$$
		\Pr[S_j = 1] = .
	$$
}
...
\begin{lemma}{
	Assuming that the error rate is independent and
	the distribution of $\Gamma(j)$ is i.i.d for all stabilizers $j$,
	$L_1, L_2, ..., L_M$ are independent and
	$$
		\Pr[L_j = 1] = .
	$$
}
