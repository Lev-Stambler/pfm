...
# Many thanks to https://cs.stackexchange.com/questions/58129/uniform-generation-of-random-bipartite-bi-regular-graphs
def createBiregularTannerGraph(regularity: tuple[int, int], n: int, m: int):
    edges = []
    leftStubs = []
    rightStubs = []
    [leftStubs.extend([node for x in range(regularity[0])])
     for node in range(0, n)]
    [rightStubs.extend([node for x in range(regularity[1])])
     for node in range(n, n + m)]

    while len(leftStubs) > 0 or len(rightStubs) > 0:
        assert(len(leftStubs) == len(rightStubs))
        l = random.randrange(0, len(leftStubs))
        r = random.randrange(0, len(rightStubs))
        edges.append((leftStubs[l], rightStubs[r]))
        del leftStubs[l]
        del rightStubs[r]
...
"""
	Have the indices from 0... n - 1 correspond to A
	and n to n + m - 1 correspond to B 
"""


def createRegularTannerGraph(regularity: tuple[int, int], n: int, m: int):
    degrees = {}

    def degreesNotMet(degrees):
        for i in range(n):
            if degrees[i] < regularity[0]:
                return True

        for i in range(n, n + m):
            if degrees[i] < regularity[1]:
                return True

    while degreesNotMet(degrees):
        for i in range(n):
            if degrees[i] < regularity[0]:
                return True
...
def gen_hx_hz(k=5, regularity=(5, 6)):
    ...
    (g1, g2) = (createBiregularTannerGraph(regularity, n, m),
                createBiregularTannerGraph(regularity, n, m))
    #(createTannerGraph(regularity, n, m), createTannerGraph(regularity, n, m))
